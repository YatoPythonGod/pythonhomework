...
lst_prog_lang = ['Python', 'c#', 'c++', 'Java', 'c', 'JavaScript', 'Go']
range_num = list(range(1, len(lst_prog_lang) + 1))


def zip_iter(*args):
    return list(map(lambda x: (x[0], x[1].upper()), zip(*args)))


def ord_filter(lst: list):
    for i in lst:
        if sum(list(map(lambda x: ord(x), list(i[1])))) % i[0] == 0:
            yield sum(list(map(lambda x: ord(x), list(i[1])))), i[1]


print(list(ord_filter(zip_iter(range_num, lst_prog_lang))))
